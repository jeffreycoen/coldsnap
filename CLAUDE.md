...
**Orientation (orchestrator only, before any work):** read the newest phase skeleton in `docs/superpowers/plans/` and the last ~20 commit subjects. Dispatched agents read only their brief's verified list. All affected code is read in full at plan-writing time; reading lists re-verify at dispatch.
...
- Markdown documents are served to the owner rendered, never as a file path.
