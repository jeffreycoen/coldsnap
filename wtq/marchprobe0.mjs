// Q: baseline plain-march probe (no dbg dependency).
...
for (let i = 0; i < 12; i++) {
  ...
  const d = await phone.evaluate(() => {
    ...
    return { v: +Math.hypot(m.hull.v.x, m.hull.v.z).toFixed(2), mode: m.state.mode, cmdF: +m.state.cmdT.f.toFixed(2) };
  });
  console.log(JSON.stringify(d));
  if (d.mode === "FALLEN") break;
}
await mt.end(1);
await browser.close();
