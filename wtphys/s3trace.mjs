...
for (let i = 0; i < 4560; i++) {
  ...
  if (t > 29 && (i % 24 === 0 || mech.state.mode === "FALLEN")) {
    const st = mech.state;
    const yn2 = Math.atan2(mech.hull.R[6], mech.hull.R[8]);
    console.log(t.toFixed(1), st.mode, "R4", mech.hull.R[4].toFixed(3), "v", Math.hypot(mech.hull.v.x, mech.hull.v.z).toFixed(2), "hdgErr", (st.heading - yn2).toFixed(2), "rec", (st.recoverT || 0).toFixed(1), "stopping", !!st.stopping, "ride", mech.magicRide == null ? 8 : mech.magicRide);
  }
