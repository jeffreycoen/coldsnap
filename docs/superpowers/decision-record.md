...
- BODY LISTS RETIRED (2026-08-13, the owner's second-strike ruling executed): the typed-pools optimization is retired after its re-measurement failed the ship rule. The defended-battle window VINDICATED the concept (sim mean −25% to −40% with a real defense fielded) but the idle window regressed in both repeats even with the cold-frame gate (+8%/+43% — the wide AFTER spread suggests measurement noise, recorded honestly), and the rule was AND across all four cells. Everything reverted clean to mk1.13; the full numbers live in the session record. If armor-era loads ever demand it, reopening requires the owner's explicit word.
