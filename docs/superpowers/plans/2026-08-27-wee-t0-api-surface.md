# War Engine Extraction — Task 0: the API surface (mk2.69)

One new file, `src/depot/api.js`. No runtime the game executes changes — the
file exports typedefs, the roster contract, the map adapter, three throwing
surface functions, and its own command line. Keystone hash and depot-test
pass count cannot move because nothing existing is touched.

Consulted plan: `docs/superpowers/plans/2026-08-27-war-engine-extraction.md`
(the API section and step 0). Phase document:
`2026-08-27-war-engine-extraction-phase.md`.

**Suggested model: Sonnet 5** — the code is written and verified below; the
task is transcription, the derivation diff, and the gates.

## Required reading (verified against the tree, 2026-08-27)

Read these in full before any edit, and open your report by confirming each
was read:

1. This task plan, whole.
2. `docs/superpowers/plans/2026-08-27-war-engine-extraction.md` — the API
   section and step 0 only.
3. `src/depot/specs.js` (305 lines) — the 21 exports the contract names.
4. `src/depot/mapgen.js` lines 1–60 — the export-let block and the constants
   the map adapter imports.
5. `src/depot/save.js` lines 155–310 — serializeFront; the Run typedef is
   exactly the S. fields it touches.
6. `src/engine/core.js` — the makeField body (search `function makeField`),
   the makeBody body (`function makeBody`), the makeWorld body
   (`function makeWorld`), and worldHash (`function worldHash`). Nothing
   else in that file.

## Pre-verification done at plan time (2026-08-27)

The code block in step 2 was syntax-checked (`node --check`, ESM) and run
against the live tree with imports path-adjusted: checkSpecs returns zero
problems on today's specs.js; mapFromGlobals throws the TOWN sentinel before
makeMap and returns a 29-key map after makeMap(11) (102 TOWN rows); runHash
is insertion-order stable; `gate` exits 1 naming step 4; an unknown argument
exits 2; `manifest` on src/ui/startview.js reports addBody, makeField,
makeWorld under engine/core.js and makeRenderer under render/renderer.js.

Browser-bundle note: no browser entry imports api.js at this task, so the
guarded `import("node:url")` at the bottom of the file is never seen by the
bundler; the browser-import question returns at step 3 and is that task's
business.
...
Every command below must give exactly the stated result, or the task stops
and the mismatch is reported as a finding.

```bash
test ! -f src/depot/api.js && echo CLEAR        # must print CLEAR
grep -oE '^export (const|function|let) [A-Za-z0-9_]+' src/depot/specs.js | wc -l   # must print 21
grep -ohE 'S\.[a-zA-Z_]+' src/depot/save.js | sort -u | wc -l                      # must print 32
```

And the tower keys, via a scratch script or a node one-liner over a dynamic
import of `./src/depot/specs.js`: `Object.keys(TOWER_SPECS)` must print
`mg gun mortar rocket tesla`.

### Step 2 — create src/depot/api.js

Write the file below verbatim. It is complete — no design decisions remain
in it.
 ...
 * coverZ, coverIn, gT, _fGrace, hitT.
 */
 ...
 * @property {number} bell @property {number} bellAt @property {number} bellT
...
### Step 3 — the derivation diff

The typedefs above were derived at plan time. Re-derive at dispatch and
reconcile; a moved number is a finding, not an error — update the typedef
and report every difference old→new.

```bash
# World game-assigned fields — plan-time result was 14 distinct:
grep -rhoE 'world\.[a-zA-Z_]+ = ' src/depot/*.js src/depot/*.jsx | sort -u
# Run fields — must equal the 32-name list in the Run typedef:
grep -ohE 'S\.[a-zA-Z_]+' src/depot/save.js | sort -u
# Body game-assigned fields — used to find MISSING names only:
grep -rhoE '\b[busv]\.[a-zA-Z_]+ = ' src/depot/*.js src/depot/*.jsx src/engine/mech.js | sort -u
```

The Body grep over one-letter receivers is noisy (it catches locals); use it
to find missing names, never to delete listed ones. Every reconciliation
edits a typedef comment only — no runtime line changes.

### Step 4 — functional asserts

```bash
node --check src/depot/api.js && echo SYNTAX-OK
node src/depot/api.js gate; echo "exit=$?"
#   must print the bootWar message naming step 4, then exit=1
node src/depot/api.js nonsense; echo "exit=$?"        # usage, then exit=2
node src/depot/api.js manifest src/ui/startview.js
#   must list addBody, makeField, makeWorld under engine/core.js and
#   makeRenderer under render/renderer.js
```

And in a scratch script (run from the repo root, never committed):

```js
import * as api from "./src/depot/api.js";
import * as SPECS from "./src/depot/specs.js";
import { makeMap } from "./src/depot/mapgen.js";
if (api.checkSpecs(SPECS).length !== 0) throw new Error("contract fails on live specs.js");
api.assertSpecs(SPECS);
let threw = false;
try { api.mapFromGlobals(); } catch (e) { threw = /TOWN is empty/.test(e.message); }
if (!threw) throw new Error("boot sentinel did not fire pre-makeMap");
makeMap(11);
const map = api.mapFromGlobals();
if (map.TOWN.length === 0 || map.MAP_SEED !== 11) throw new Error("mapFromGlobals wrong after makeMap");
if (api.runHash({ b: 1, a: [2, { d: 3, c: 4 }] }) !== api.runHash({ a: [2, { c: 4, d: 3 }], b: 1 })) throw new Error("runHash unstable");
console.log("T0 functional asserts: PASS");
```

### Step 5 — tree state

`git status` must show exactly one new source file (`src/depot/api.js`)
beyond the version bump. Anything else is a stop.

### Step 6 — version, gates, landing

1. Bump `src/version.js` MK to `"mk2.69"`. Build AFTER the bump.
2. `node scripts/gate.mjs depot-test` — 2,091 PASS / 0 FAIL (the baseline;
   a moved count here is a stop, not a finding).
3. `node scripts/gate.mjs golden` — ALL PASS.
4. Gates green → commit → push. Commit subject:
   `the surface laid — api.js, the war engine's one door, mk2.69`

## Acceptance (arithmetic)

- depot-test: 2,091 PASS / 0 FAIL, unchanged.
- golden: ALL PASS.
- `node src/depot/api.js gate` exits 1 and its message names step 4.
- checkSpecs on the live specs.js: 0 problems.
- One new source file in the tree.

## Report

One line of outcome, then: the seeds the gates ran (depot-test's own pinned
fixture seeds; no seed is special), every derivation finding old→new, every
re-teach (none expected — no pinned text moves), any skipped step named as
skipped.
