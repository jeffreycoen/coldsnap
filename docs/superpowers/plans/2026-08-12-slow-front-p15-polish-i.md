...
3. **BUILD order** — game-layer machinery (economy stays OUT of squads.js per its module law):
   - Engineer squads get a BUILD chip beside DEFEND/ATTACK/MOVE (engineer squads only). BUILD arms the ground tap like MOVE; the tapped dest clamps to the rim (Task 1's clamp).
   - Under order "build": the squad travels on the MOVE legs (quiet, threat read forced false — reuse, don't fork, the mk0.28 move semantics in `stepSquad`); squads.js only knows "build" moves like "move".
   - The PLACEMENT of bags is the game layer's: in `stepDepot`, when a squad under "build" has advanced `BAG_SPACING` (~2.5m) along its path since the last bag, place a sandbag at the trailing member's position (`spawnSandbag` + `sandbagOrientAt` auto-orient), spend `SANDBAG_FIELD_COST` (3 — cheaper than the menu's 5), and stop placing when scrap runs dry (squad keeps walking). Arrival flips to defend as MOVE does.
