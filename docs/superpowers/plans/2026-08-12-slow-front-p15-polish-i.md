...
**Task 4 — The engineers arrive (mk0.60).** *(Redesigned 2026-08-12 after discussion with Jeff; supersedes the walk-and-drop version. Depot evening/placement work is HELD for The Front phase — none of it ships here.)* Every match now starts with **rifles and an engineer team**, and a few sandbags already seeded around the depot. Engineers take a **two-point BUILD order**: tap where the line starts, tap where it ends, and the squad walks to the start and lays **end-to-end along the line** to the end, then digs in there. Each piece takes the closest logical rotation to fill the line — straight north-south or east-west lines lay perfectly end-to-end; diagonal lines become short alternating runs (the engine's boxes can't sit at an angle — real rotation waits for a phase that needs it broadly). Engineers lay **bags or walls**: field bags cost 3 (menu 5), field walls cost 5 (menu 8) but the squad pauses at each wall — a wall line under fire is a commitment. Scrap runs dry: they stop laying and keep walking. Repair and bridges stay in their full phase later.
...
3. **Two-point BUILD order** — game-layer machinery (economy stays OUT of squads.js per its module law):
   ...
   - **Line rasterization is the game layer's** (in `stepDepot`): walk the start→end segment through grid cells (staircase on diagonals). Each cell along the run gets its piece placed when the squad's anchor has advanced past it, oriented to the LOCAL step direction (the closest logical rotation — N-S/E-W runs lay perfectly end-to-end; the existing `sandbagOrientAt`/`wallOrientAt` auto-continue conventions are the fallback, but the step direction is primary). END-TO-END is the goal: consecutive bags in a straight run must touch, per the bag's long axis vs the grid pitch — read the actual cell size and bag dims and reconcile; report the geometry you land on.
