...
**AMENDMENT 3 (2026-08-20, from the owner's live check; ships as mk1.91).** Three fixes.

1. **Picked cards now light up, and tapping the same card twice un-picks it instead of eating a second slot.» The cause: the screen remembered which card objects were picked, but the screen's own refresh replaces those objects eight times a second, so a picked card never matched itself again — no highlight, and every re-tap counted as a new pick. The fix: the screen remembers picks by the card's NAME, which never changes. Same card tapped again = un-picked. Five different cards is the only way to fill five slots.
2. **The starting money is truly 250 now.** The game sets its opening money in its own startup code, not through the settings file my earlier step changed — so the live game still opened at 120. That startup number moves to 250.
3. **Proof the draft is free, in the tests.** A new check reads the placement code and fails if any line in it ever touches your money. Picking is free, placing a drafted unit is free, and a drafted plan opens its build-bar slot free. (Building MORE of a plan's type from the bar afterward still costs the market price — that is the ruled economy, unchanged.)

The exact edits: in the draft screen, the picked list holds `c.k` (the card name) — `toggle(c.k)`, highlight on `picked.includes(c.k)`, and confirm passes `cards.filter((c) => picked.includes(c.k))`. In the game's startup code, the line `resources: 120,` (near line 1256) becomes `resources: 250, // the draft's richer opening (owner) // provisional (F5)`. Three new checks join the test block: the pick-by-name shape, the 250 startup line, and the free-placer proof (the placePick function's body contains no `S.resources` change). Expected count: 1615 → **1618/0**. Version mk1.91, commit subject exactly `the draft picks hold (mk1.91)`; the mech becomes mk1.92.
