...
## T6 — the ending's dignity + safe exits (mk0.30; approved 2026-08-11)
Three items, one commit:
1. **RETURN TO BASE dead — DIAGNOSE FIRST.** The end card's button (Dispatch, gating=false, label "RETURN TO BASE", onAcknowledge → onExit || restart) does nothing per the director's playtest. The between-wave ACKNOWLEDGE works, so suspects: the 500ms arming timer racing the end-card mount, the onExit prop wiring from App, or a recent regression (check the tap-theft T3 changes if they landed first — same machinery). Find the cause, report it, fix minimally.
2. **Savor the fall:** on either breach verdict, DELAY the end card ~6s (world keeps simming and rendering — the collapse plays out; provisional feel number, Jeff tunes by play). Implementation shape: breach sets the verdict flags as today, but the card mount gates on `world.t - S.endedAt >= END_CARD_DELAY_S` (new state stamp; deterministic, no rng). HUD stays; input to build/order locks at verdict (the war IS over — only the camera and the watching remain).
3. **MENU two-tap arm:** the NEW CAMPAIGN pattern (CampaignScreen's arm/disarm, 5s timeout) applied to the in-game ⏏ MENU button: first tap → red "LEAVE THE FIELD?", second tap within 5s exits, timeout disarms. Same for any other instant-exit surface in DEPOT (audit: the end card's RETURN TO BASE needs NO arm — the war is over).
- [ ] Failing asserts where headless-testable (endedAt stamp + delayed dispatch mount state; input lock at verdict); browser checks for the button fix + arm flow (data- attributes); reading: Dispatch.jsx (arming), DepotGame end-card render + onExit wiring from App.jsx, CampaignScreen reset arm pattern, T3's changes. Traps: the trailing-tap lesson (any card mounting on a tap eats the next click — the delay actually HELPS here); rotation-proof screen-space chips.
