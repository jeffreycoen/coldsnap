...
**AMENDMENT 3 (2026-08-14, owner's ruling — "okay, ship it"):** the five-per-side idle measurement found the idle cost REAL but bounded: +0.05 ms mean (1.966 → 2.016, +2.5%), consistent in mean and median. The owner, with the trade explained plainly — battles twice as cheap against an invisibly small quiet-map cost with 14 ms of slack — rules: SHIP. The idle gate is struck for this task; the ship rule is the defended window's standing PASS (12.75/12.34 → 6.60/6.68) plus the idle cost recorded honestly as accepted. Re-land Steps 1-8 (third identical landing), run the remaining gates, ship mk1.19, run Step 10's ramps.
