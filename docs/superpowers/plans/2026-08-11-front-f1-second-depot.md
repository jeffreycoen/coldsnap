...
- ~~Blast shove vs welded masonry is the demolition mechanism (enemy sappers already breach player walls this way — kv 9 pops welds and scatters stones past the 1.2m standing tolerance)~~ **PREMISE FALSIFIED (measured 2026-08-11):** walls die to the charge's DAMAGE number (they have hp); depot stones have no hp and the kv 9 blast peaks at 89,268 force vs the depot's 120,000 joint strength — 60 teams left depot2 99.1% standing. **Jeff's decision: BIGGER DEMOLITION CHARGE.** The satchel spec (both sides — one charge, symmetric) is raised until a charge planted at the sapper's REAL stopping distance (hx + 1.3) measurably pops joints and scatters stones, tuned by measurement: find the minimal {r, kv} that yields breach in a bounded number of teams (target: a run's affordable count, single digits to low teens — record the curve, pick the knee, label provisional F5). Mandatory side-measurements before commit: the bigger charge vs walls/towers/units/own-team splash (enemy sappers use the same satchel on YOUR walls — record the delta, flag if wall fights change beyond recognition), and the enemy-sapper-vs-player-depot threat that now becomes real (report it plainly to Jeff — his depot is now demolishable too).
