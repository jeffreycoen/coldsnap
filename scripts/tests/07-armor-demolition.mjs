...
// ==== P7 T3: THE SEAT OF THE WAR ============================================
{
  ...
  const simSrcT3b = fs.readFileSync(new URL("../../src/depot/sim.js", import.meta.url), "utf8");
  const sliceFn4 = (name) => {
    let start = srcT3.indexOf(`\nfunction ${name}(`), rest;
    if (start >= 0) { rest = srcT3.slice(start + 1); }
    else {
      start = mgSrcT3b.indexOf(`\nexport function ${name}(`);
      if (start >= 0) { rest = mgSrcT3b.slice(start + 1).replace(/^export /, ""); }
      else {
        start = simSrcT3b.indexOf(`\nexport function ${name}(`);
        if (start < 0) throw new Error("T3 extract: missing function " + name);
        rest = simSrcT3b.slice(start + 1).replace(/^export /, "");
      }
    }
  ...
  const mapSrcT3 = [
    headerT3,
    sliceFn4("formOf"), sliceFn4("layDressing"), sliceFn4("stoneCount"), sliceFn4("genMap"), "const liveGameMap = () => null; // task-2a harness stub: sliced makeMap's return, unused here", sliceFn4("makeMap"), sliceFn4("streamAt"), sliceFn4("pondAt"), sliceFn4("rockAt"),
...
// ==== P7 T5: THE PRECAST DEPOT AND THE HONEST RESUME ========================
{
  ...
  // (census/standing/breach) flows through the new pieces unchanged.
  {
    ...
    const simSrcT5b = fs.readFileSync(new URL("../../src/depot/sim.js", import.meta.url), "utf8");
    const sliceFn5 = (name) => {
      let start = srcT5.indexOf(`\nfunction ${name}(`), rest;
      if (start >= 0) { rest = srcT5.slice(start + 1); }
      else {
        start = mgSrcT5b.indexOf(`\nexport function ${name}(`);
        if (start >= 0) { rest = mgSrcT5b.slice(start + 1).replace(/^export /, ""); }
        else {
          start = simSrcT5b.indexOf(`\nexport function ${name}(`);
          if (start < 0) throw new Error("T5 extract: missing function " + name);
          rest = simSrcT5b.slice(start + 1).replace(/^export /, "");
        }
      }
      const m = rest.slice(9).search(/\n(?:function |export |const [A-Z])/);
      return rest.slice(0, m < 0 ? rest.length : m + 9);
    ...
    const mapSrc5 = [
      header5,
      sliceFn5("formOf"), sliceFn5("layDressing"), sliceFn5("stoneCount"), sliceFn5("genMap"), "const liveGameMap = () => null; // task-2a harness stub: sliced makeMap's return, unused here", sliceFn5("makeMap"), sliceFn5("streamAt"), sliceFn5("pondAt"), sliceFn5("rockAt"),
...
// ==== P7 T6: THE DEFENSIVE OPENING ==========================================
{
  ...
  {
    const bag = ["gren", "", "rocket", "sapper", "", "mortar", "sniper", ""];
    const picked = pickHomeDetail(bag, 3);
    ok("T6(b): picks the first three rifle-family tags", picked.join(",") === ",rocket,", JSON.stringify(picked));
  ...
  // battering (20+ blasts) still gets there — hard, not impossible.
  {
    ...
    const simSrcT6b = fs.readFileSync(new URL("../../src/depot/sim.js", import.meta.url), "utf8");
    const sliceFn6 = (name) => {
      let start = srcT6.indexOf(`\nfunction ${name}(`), rest;
      if (start >= 0) { rest = srcT6.slice(start + 1); }
      else {
        start = mgSrcT6b.indexOf(`\nexport function ${name}(`);
        if (start >= 0) { rest = mgSrcT6b.slice(start + 1).replace(/^export /, ""); }
        else {
          start = simSrcT6b.indexOf(`\nexport function ${name}(`);
          if (start < 0) throw new Error("T6 extract: missing function " + name);
          rest = simSrcT6b.slice(start + 1).replace(/^export /, "");
        }
      }
      const m = rest.slice(9).search(/\n(?:function |export |const [A-Z])/);
      return rest.slice(0, m < 0 ? rest.length : m + 9);
    ...
    const mapSrc6 = [
      header6,
      sliceFn6("formOf"), sliceFn6("layDressing"), sliceFn6("stoneCount"), sliceFn6("genMap"), "const liveGameMap = () => null; // task-2a harness stub: sliced makeMap's return, unused here", sliceFn6("makeMap"), sliceFn6("streamAt"), sliceFn6("pondAt"), sliceFn6("rockAt"),
...
// ==== P7 T7: ROCKETS AND GRENADIERS (re-taught mk2.02 — the roster surgery) =
{
  const flatF = { heightAt: () => 0, dirty: false, carve: () => {}, normalAt: (x, z, o) => { o.x = 0; o.y = 1; o.z = 0; return o; } };
  ...
  // (b), the runner-speed check, died with the runners (mk2.02, licensed).
  const time = (type) => {
  ...
  const RIFLES_PIN_STEPS = 1389; // re-captured mk2.02 (named): the 2m body moves the march's contact arithmetic; was 1425
  const tR = time("rifles");
  ok("T7(c) (re-captured mk2.02): rifles march the same 30m in the exact pinned step count (fallback exactly 3.2)",
    tR === RIFLES_PIN_STEPS, `${tR}`);
  // (d)/(e), the breaker grind and ram suite, died with the breakers
  // (mk2.02, licensed — the ram mechanism is deleted).
  // (f) the ungated pool still deals eighteen, the new keys included
  {
    ok("T7(f) (re-taught mk2.02): the ungated plans pool at bell one is twenty — rockets and grenadiers included", (() => { const p = HAND_KEYS.filter((k) => makeManifestState().unlocked.indexOf(k) < 0); return p.length === 20 && p.includes("sq_rockets") && p.includes("sq_grenadiers"); })());
  }
  ...
  // the same family (marketCounts merges them)
  {
    ...
    spawnUnit(w, { x: 0, z: 0 }, "rocket"); // one enemy rocket man
    const sq = makeSquad(2, "rockets", 1, 10, 10);
    spawnSquadMembers(w, sq); // two player rocket men
    const counts = mkt.marketCounts(w, [sq]);
    ok("T7(g) (re-taught mk2.02): the rocketteam family counts both armies' men", counts.rocketteam === 3, `rocketteam=${counts.rocketteam}`);
  }
  ...
  // new types.
  {
    ...
    ok("T7(h) (re-taught mk2.02): a rocket team's possessedVolley fires", firedR > 0, `fired=${firedR}`);

    const bsq = makeSquad(2, "grenadiers", 1, 0, 0);
    spawnSquadMembers(w, bsq);
    w.events.length = 0;
    const firedB = possessedVolley(w, bsq, { x: 10, z: 0 }, null);
    ok("T7(h) (re-taught mk2.02): a grenadier squad's possessedVolley fires", firedB > 0, `fired=${firedB}`);
  }
}
// ==== end P7 T7 ==============================================================
...
//  (e) the fielded start: a player rocket team + grenadier squad on defend
...
//      boots (hash equality of the twin worlds)
{
  ...
  // (b) planWave never shops heroes; nextSpawnTag never yields one
  {
    ...
    const tags9b = ["", "rocket", "mortar", "gren", "sapper", "sniper", "tank", "hero_bison", "hero_apc"];
