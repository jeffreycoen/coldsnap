...
export default function App() {
  ...
  useEffect(() => {
    if (!GAME_SCREENS.has(screen) && screen !== "campaign" && screen !== "mechrange") return; // the order book and the mech range exit on ESC too (range stays out of GAME_SCREENS: it reads raw key codes, no remap)
  ...
  return <StartScreen onPlay={() => setScreen("demo")} onSandbox={() => setScreen("sandbox")} onCampaign={() => setScreen("campaign")} onControls={() => setScreen("controls")} onMech={() => setScreen("mechrange")} />;
}
