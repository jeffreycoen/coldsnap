...
// trials — text only, zero mechanics change. Saves live under coldsnap-camp-*
...
export default function CampaignRunner({ entry, onExit, onComplete }) {
  ...
  useEffect(() => {
    if (hud.trial && hud.trial.free && !hud.aar) {
      const t = setTimeout(() => { if (onExit) onExit(); }, 900);
      return () => clearTimeout(t);
    }
  }, [hud.trial && hud.trial.free, hud.aar]);
  ...
  useEffect(() => {
    ...
    (async () => {
      const t = await loadTally("coldsnap-camp-tally");
    ...
    const advanceTrial = (skipped) => {
      ...
      if (t) {
        ...
        let desc = TRIALS[S.trial.idx + 1] ? "Next: " + TRIALS[S.trial.idx + 1].title : "Order complete. Returning to the order book.";
        if (!skipped) {
          const el = Math.max(0.1, S.world.t - S.trial.t0);
          const m = MEDAL(t, el);
          title += ` · ${el.toFixed(1)}s ${m === "GOLD" ? "★GOLD" : m === "SILVER" ? "☆SILVER" : "BRONZE"}`;
          const c = t.commendation ? { wo: spec.contract.wo, title: spec.contract.title, commendation: t.commendation, directive: t.hint } : CONTRACTS[t.id];
    ...
    const trialLoad = async () => {
      try {
        const rm = await window.storage.get("coldsnap-camp-medals");
        const mm = JSON.parse(rm.value);
        if (mm && typeof mm === "object") S.medals = mm;
      } catch (e) {}
      try {
        if (S.running) enterTrial(0); // one order per deployment, no mid-order resume
    ...
    const persistSave = () => {
      ...
      try { window.storage.set("coldsnap-camp-ach", JSON.stringify({ unlocked: [...a.unlocked], total: a.total })); } catch (e) {}
    };
    ...
    const stateSave = () => {
      ...
      try { window.storage.set("coldsnap-camp-tally", JSON.stringify(cur.tally)); } catch (e) {}
    };
    ...
    const doReset = () => {
      ...
      S.world = buildScenario(spec, { worldSeed: (spec.terrain.worldSeed || 1234) + S.resets, shelters: true });
              ...
              return (
                <span
                  ...
                  onClick={async () => {
                    ...
                    if (!S || !m) return;
                    try {
                      const r = await window.storage.get("coldsnap-camp-aar-" + t.id);
