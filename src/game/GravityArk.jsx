...
import ArkHud from "./gravityark/hud.jsx";
...
export default function GravityArk({ onExit }){
  ...
  useEffect(()=>{
    ...
    const loop=()=>{
      ...
      const baseSc=calcScale(W,H,allPts)*2,sc=baseSc*cam.zoom;// start zoomed in — the whole field no longer fits (2.0 is a design choice until played)
      // The camera rests on the ship; the field is bigger than the frame now
      const lookX=sh.x,lookZ=sh.z;
  ...
  return(
    <div style={{position:"fixed",inset:0,overflow:"hidden",background:"#f5f4f0",fontFamily:ff}}>
      ...
      <ArkHud {...{ ui, s, locked, noFuel, showTutorial, setShowTutorial, progress, lock, ffRef, setUi, doLaunch, doPlan, doExec, doCancel, doRetry, doNext, toggleMap, copyLogs, exportFavorites, rateLevel, adjustAngle, adjustPower, onExit }}/>
    </div>);
}
